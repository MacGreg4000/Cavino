...
import { FileJson, Plus, AlertTriangle, CircleAlert } from 'lucide-react';
...
import { flattenWineImport } from '../lib/flattenWineImport';
import { validateWineImportFile, type WineImportFile } from '../types/wineImport';

const WINE_TYPES = ['Rouge', 'Blanc', 'Rosé', 'Champagne', 'Effervescent'];

export function AddWineImport() {
  const navigate = useNavigate();
  const { toast } = useToast();
  const createWine = useWineStore((s) => s.createWine);

  const [fileErrors, setFileErrors] = useState<string[]>([]);
  const [importData, setImportData] = useState<WineImportFile | null>(null);
  const [flattened, setFlattened] = useState<Partial<Wine> | null>(null);
  ...
  const checkDuplicate = async (n: string, d?: string, v?: number) => {
    if (!n.trim()) return;
    try {
      const res = await apiFetch(`/api/wines?search=${encodeURIComponent(n.trim())}`);
      if (!res.ok) return;
      const candidates: Wine[] = await res.json();
      const match = candidates.find(
        (w) => (w.domain ?? '').toLowerCase() === (d ?? '').toLowerCase() && w.vintage === v
      );
      if (match) {
        setDuplicateWarning(
          `Un vin similaire existe déjà : ${match.name}${match.vintage ? ` (${match.vintage})` : ''} — ${match.quantity ?? 0} bouteille(s).`
        );
      } else {
        setDuplicateWarning(null);
      }
    } catch {
      // best-effort, on ne bloque jamais l'import pour ça
    }
  };
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setFileErrors(['Fichier JSON invalide ou corrompu.']);
      return;
    }

    const errors = validateWineImportFile(parsed);
    if (errors.length > 0) {
      setFileErrors(errors);
      return;
    }

    const data = parsed as WineImportFile;
    const flat = flattenWineImport(data);

    setImportData(data);
    setFlattened(flat);
    setName(flat.name ?? '');
    setDomain(flat.domain ?? '');
    setType(flat.type ?? '');
    setVintage(flat.vintage != null ? String(flat.vintage) : '');
    setAppellation(flat.appellation ?? '');
    setRegion(flat.region ?? '');
    setCountry(flat.country ?? '');
    setBottleSize(flat.bottleSize ?? '75');
    setPurchasePrice(flat.purchasePrice ?? '');

    if (data.photo?.dataUrl) {
      setPhotoPreview(data.photo.dataUrl);
    }

    checkDuplicate(flat.name ?? '', flat.domain, flat.vintage);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Upload de la photo issue du fichier importé, si présente
      if (importData?.photo?.dataUrl && wine.id) {
        try {
          const photoFile = dataUrlToFile(importData.photo.dataUrl, importData.photo.filename || 'wine.jpg');
          ...
          toast('error', "Vin créé mais l'upload de la photo a échoué");
        }
      }
  ...
  return (
    <div>
      <PageHeader title="Importer un fichier" back />
          ...
          <label className="cursor-pointer block">
            ...
            {photoPreview ? (
              ...
              <div className="aspect-[4/3] bg-surface-hover rounded-[var(--radius-md)] flex flex-col items-center justify-center gap-2 border-2 border-dashed border-border hover:border-accent transition-colors">
                ...
                <span className="text-sm text-text-muted">Choisir le fichier JSON</span>
              </div>
            )}
          </label>
          {importData && (
            <p className="text-xs text-text-muted mt-2 text-center">
              Fichier chargé — modifie les champs ci-dessous si besoin avant de valider.
            </p>
          )}
        </Card>
        ...
        {duplicateWarning && (
          <Card className="border-gold/40 bg-gold/5">
            <div className="flex items-start gap-2">
              ...
              <p className="text-sm text-gold">{duplicateWarning}</p>
            </div>
          </Card>
        )}
